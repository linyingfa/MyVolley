 ...
 */
public class ImageLoader {//ImageLoader 其实是对 ImageRequest 的封装，它不仅可以帮我们对图片进行缓存，
// 还可以过滤掉重复的链接，避免重复发送请求，因此 ImageLoader 要比 ImageRequest 更加高效。
    /**
    ...
    public ImageContainer get(String requestUrl, ImageListener imageListener, int maxWidth, int maxHeight, ScaleType scaleType) {
        ...
        // 判断该请求是否是否在缓存队列中, Url 从缓存队列中取出 Request
    ...
    /**
     * ImageLoader 其实是对 ImageRequest 的封装，它不仅可以帮我们对图片进行缓存，
     * 还可以过滤掉重复的链接，避免重复发送请求，因此 ImageLoader 要比 ImageRequest 更加高效。
     *
     * @param requestUrl
     * @param maxWidth
    ...
    protected Request<Bitmap> makeImageRequest(String requestUrl, int maxWidth, int maxHeight, ScaleType scaleType, final String cacheKey) {
        return new ImageRequest(requestUrl, new Listener<Bitmap>() {
            ...
            public void onResponse(Bitmap response) {
                //网络请求成功之后，调用 onGetImageSuccess() 方法，将 Bitmap 进行缓存，
                // 以及将缓存队列中 cacheKey 对应的 BatchedImageRequest 移除掉，最后调用 batchResponse() 方法。
