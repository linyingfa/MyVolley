...
public class ImageRequest extends Request<Bitmap> {
    ...
    private static int getResizedDimension(int maxPrimary, int maxSecondary, int actualPrimary, int actualSecondary, ScaleType scaleType) {
        ...
        //构建 ImageRequest 的时候传入的参数吧，那 6 个参数里面，包含两个分别指定图片最大宽和高的参数，
        // 我们将传入的图片最大宽和高以及 Bitmap 真实的宽和高，通过 getResizedDemension() 方法计算出比较合适的图片显示宽高
    ...
    private Response<Bitmap> doParse(NetworkResponse response) {
        ...
        if (mMaxWidth == 0 && mMaxHeight == 0) {
            ...
            //TODO  ① 获取 Bitmap 原始的宽和高
            //通过 BitmapFactory 将传入的 NetworkResponse 中的 data 转换成对应的 Bitmap，
            // 然后通过设置 BitmapOptions.inJustDecodeBounds = true，得到 Bitmap 的原始宽和高，这里补充一下，
            // 当  BitmapOptions.inJustDecodeBounds = true 的时候，BitmapFactory.decode 并不会真的返回一个 bitmap 给你，
            // 它仅仅会把一些图片的大小信息（如宽和高）返回给你，而不会占用太多的内存。
            ...
            //TODO ③ 根据我们想要的宽和高得到对应的 Bitmap
            decodeOptions.inJustDecodeBounds = false;
            // TODO(ficus): Do we need this or is it okay since API 8 doesn't support it?
            // decodeOptions.inPreferQualityOverSpeed = PREFER_QUALITY_OVER_SPEED;
            //DecodeOptions.inJustDecodeBounds = true 代表将一个真正的 Bitmap 返回给你，
            // DecodeOptions.inSampleSize 代表图片的采样率，是跟图片压缩有关的参数，
            // 如果 inSampliSize  = 2 则代表将原先图片的宽和高分别减小为原来的 1/2，以此类推。
            decodeOptions.inSampleSize = findBestSampleSize(actualWidth, actualHeight, desiredWidth, desiredHeight);
            Bitmap tempBitmap = BitmapFactory.decodeByteArray(data, 0, data.length, decodeOptions);
            // If necessary, scale down to the maximal acceptable size.
            //TODO ④ 如果 Bitmap 不为 bull 而且宽或高大于目标宽高的话，再一次压缩
        ...
        //TODO ⑤ 将得到的 包含 Bitmap 的 Response 回调出去
        if (bitmap == null) {
